...
double gammaD(1e5),pG1(1e5),pG2(1e5),deltaG1(1e5),deltaG2(1e5),slopeG1(1e5),slopeG2(1e5),slopeG12(1e5),initialDisc(1e5),\
A_N_L_end(1e5),B_N_L_end(1e5),A_N_R_end(1e5),B_N_R_end(1e5),C_N_L_end(1e5),C_N_R_end(1e5),tailFactor(1e5),angle_two_phi(100.),\
angle_delta(100.),expG2(1e5),expG1(1e5), y_gridExp(1e5), abs_f(1e5);
int BC_switch(10000);
...
/////////////////////////START: ///////////////////////////// save results in file //////////////////////////////////
	...
	double xSave(1e5),ySave(1e7),d1SOR1Save(1e3),deltaSave(1e7);	
...
gnuOutputShape = "set out 'syntecticPaperPreparePics/epsOf_"+shapeFileName+".eps"+"';";
...
gnuOutputShape = "set out 'syntecticPaperPreparePics/epsOf_closeUp_"+shapeFileName+".eps"+"';";
gnuClosing = "set terminal x11;set out";
completeGnuFileShape = gnuFileBasisShape+gnuOutputShape+plot1+gnuClosing;
...
gnuOutputCheck = "set out 'syntecticPaperPreparePics/epsOf_Check_"+shapeFileName+".eps"+"';";
gnuClosing = "set terminal x11;set out";
...
//////////////////////////////////////////////////////////////// END automated eps-generation	
	...
	for(int k = 0; k<(numberGridPoints+1) ; k++) // for calculation of elastic Integrals on Ivantsov solution comment out
	{
		...
		{	
			...
			outShapeLoadFile << xSave << " " << ySave << endl; 
			outCheckFile << xSave << " " << ySave << " " << curvature.at(k) << " " << integrand.at(k) << endl;
			//cout << " k= " << k << " xSave=" << xSave << " ySave= " << ySave << endl; 
		}
		else 
		{
	...
	outShapeFile << " ### d1S/R1 | delta  | dG1 | dG2 ### " << endl;
 	outShapeLoadFile << " ### d1S/R1 | delta  ### " << endl;
	outCheckFile << " ### d1S/R1 | delta  ### " << endl;
	outShapeFile << " ### " << d1SOR1Save << "     " << deltaSave << "     " << deltaG1 << "   " << deltaG2  <<" ### " << endl;
	outShapeLoadFile << " ### " << d1SOR1Save << "     " << deltaSave <<  " ### " << endl;
	outCheckFile << " ### " << d1SOR1Save << "     " << deltaSave <<  " ### " << endl;
...
void loadSolution(gsl_vector* x,double& discretization)
{
	...
	double sigmaInitialGuess(1.0),deltaInitialGuess(angle_delta);
	...
	if ( (solutionVector.size() != numberGridPoints+1) || (oldGridVector.size() != solutionVector.size()))	// the solution does not explicitly contain y= 0 as first element, but Grid_y does !
	{
		std::cout << " les nombres du points de la grille ne coincident pas, prends la grille sans solution anterieure " << endl;
		cout << " solutionVector.size(): " << solutionVector.size() << " num. Gridpoints: " <<  (numberGridPoints) << endl;
...
void setNumericalParameters(double& initialDiscretization, double& error_tol, string& gridSwitch, double& p1_o_p2)
{
	...
	angle_two_phi = 4.712389; // opening angle exactly 270 degrees
...
// for meiron problem, since slopes are not changed, not necessary yet to set prolongued splines for calculated slope in each step.
	...
	double dydx(0.),yObs(0.),y_0(1e5),y_m1(1e5),y_p1(1e5),kappa(0.),/*kappaHMK(0.),*/d2ydx2(0.),sigma(1e5);
	double xObs(0.);
	double Integral(100.),error(1e5), leftIntegral_locEq(10.),rightIntegral_locEq(10.),leftIntegral_conservationLaw(10.),rightIntegral_conservationLaw(10.),conservationLawIntegral(10.),locEqIntegral(10.);
	...
	//cout << " before update spline, A_N_i_end i= L, R: " << A_N_L_end << " " << A_N_R_end << endl; 
	...
	//cout << " after update spline, A_N_i_end i= L, R: " << A_N_L_end << " " << A_N_R_end << endl; 
	//exit(1);
...
///////////////////////////////////////////////////////////////
	...
	//equation[0] = (1e-7)*( gsl_spline_eval(current_splineG1,xGridG1[(numberGridPoints+1)/2+addGridPoints-1],current_accelG1) - (-0.5*xGridG1[(numberGridPoints+1)/2+addGridPoints-1]*xGridG1[(numberGridPoints+1)/2+addGridPoints-1]))\
				   /* *( gsl_spline_eval(current_splineG1,xGridG1[(numberGridPoints+1)/2+addGridPoints-1],current_accelG1) - (-0.5*xGridG1[(numberGridPoints+1)/2+addGridPoints-1]*xGridG1[(numberGridPoints+1)/2+addGridPoints-1])) */; 
	//equation[numberGridPoints] = ( gsl_spline_eval(current_splineG1,xGridG1[(numberGridPoints+1)/2+addGridPoints-1],current_accelG1)/gsl_spline_eval(current_splineG2,xGridG2[addGridPoints],current_accelG2) )-1.;
...
void initializeSolutionVector(gsl_vector* x, double& p1_o_p2)
{
	double initialGuess[numberGridPoints+1], sigmaInitialGuess(0.3),deltaInitialGuess(angle_delta);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////	
	...
	for (list<double>::iterator iter = shortGrid.begin(); iter != shortGrid.end(); ++iter)
	{
		if (*iter <= 0.)
		{
			...
			//cout << " in initializeSolutionAndVector: *iter < 0 : " << *iter << " " << solutionG1_G2.back() << endl;
		}
		else if (*iter >= 0.)
		{
	...
	for ( list<double>::iterator iter = solutionG1_G2.begin(); iter != solutionG1_G2.end(); ++iter )
	{
		...
		if (xGridIndex < (numberGridPoints+1))
		{
			...
			//cout << "in initializeSolutionAndVector: initialGuess[i]= " << initialGuess[xGridIndex] << endl; // initialGuess is correct
		}
		++counter;
		//exit(1);	
	}	 
...
void setFileNames(string& pathName, string& shapeFileName)
{
	...
	string n = IntToString(numberGridPoints+1);
	...
	string val_abs_f = DoubleToString(abs_f);
	//string goodConverge = IntToString(goodConvergeSwitch); // goodConvergeSwitch = 1 means good Convergence
//////////////////////////////////////////////////////////
/////////// change path  /////////////////////
	pathName = "syntecticRunsPrepareForPaper/";

	string configuration = Ver+"_n_"+n+"_disc_"+Disc+"_expG1_"+valExpG1+"_expG2_"+valExpG2+"_pG1_"+PG1+"_pG2_"+PG2+"_rG1S12_"+rGap1S12+"_rG2S12_"+rGap2S12+"_2Phi_"+twoPhi+"_slopeG1_"+tipS1+"_slopeG2_"+tipS2+"_d1SOR1_"+d1SOR1+"_delta_"+delt+"_abs_f_"+val_abs_f+".dat";
...
++nrAccess;
	...
	//cout << " in updateSplines: before defineFittingParabola: A_N_i_end, i=L, R " << A_N_L_end << " " << A_N_R_end << endl; 
	
	defineFittingParabola(x);

	//cout << " in updateSplines: after defineFittingParabola: A_N_i_end, i=L, R " << A_N_L_end << " " << A_N_R_end << endl;
...
////////////////////////////// for right branch x>0 //////////////////////
	...
	//cout << " in defineFittingParabola: before A_N_R.at: gsl_vector x: " << endl;
	//double x_len = x->size;
	//double x_len = numberGridPoints+1;
	//cout << " size is " << x_len << endl;
	//for (int i=0; i<x_len; i++)
	//{
	//	cout << "i, x[i] = " << i << " " << gsl_vector_get(x,i) << endl; 
	//}
	//cout << " end " << endl; 
	//exit(1); // x is everywhere 0 !!
...
void createDirectory()
{
	char commandString1[255] = "mkdir ";
	char commandString2[255] = "mkdir ";
	char commandString3[255] = "mkdir ";
	char argumentString1[255] = "syntecticRunsPrepareForPaper";
	char argumentString2[255] = "syntecticPaperPreparePics";
	char argumentString3[255] = "loadFiles";
	
	strcat( commandString1, argumentString1);
	cout << commandString1 << endl;
	system( commandString1 );
	
	strcat( commandString2, argumentString2);
	cout << commandString2 << endl;
	system( commandString2 );
	
	strcat( commandString3, argumentString3);
	cout << commandString3 << endl;
