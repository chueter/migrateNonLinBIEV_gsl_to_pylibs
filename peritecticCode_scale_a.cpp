...
//	{
//		cout << "i, xGridG1[i], prolonguedYGridG1[i] " << i << " " << xGridG1[i] << " " << prolonguedYGridG1[i] << endl;
//	}
...
void loadSolution(gsl_vector* x,double& discretization)
{
	...
	if ( (solutionVector.size() != numberGridPoints+numberGridPoints_y-1) || (oldGridVector.size() != solutionVector.size()))	// the solution does not explicitly contain y= 0 as first element, but Grid_y does !
	{
		...
		cout << " solutionVector.size(): " << solutionVector.size() << " num. Gridpoints: " <<  (numberGridPoints+numberGridPoints_y-1) << endl;
...
void initializeSolutionVector(gsl_vector* x, double& p1_o_p2)
{
	double initialGuess[numberGridPoints+numberGridPoints_y-1], sigmaInitialGuess(.4),lambdaInitialGuess(lambda);
...
//////////////////////////////////////////////////////////////
		...
		for (int yGridIndex=1; yGridIndex < numberGridPoints_y+addGridPoints; ++yGridIndex)
		{
			...
			yGrid.push_back(3.5*pow(yValue,1.0));
		}
...
// for meiron problem, since slopes are not changed, not necessary yet to set prolongued splines for calculated slope in each step.
	...
	lambda = abs(gsl_vector_get(x,numberGridPoints));
...
///////////////////////////////////////////////////////////////
		...
		equation[i] = 0.5*(deltaG1-kappa*sigma/lambda)-(pG1*lambda/(2.*PI))*Integral;
...
//////////////////////////////////////////////////////////////////////
		...
		equation[i+numberGridPoints] = -0.5*DcLDelta_DcLGamma*kappa*sigma/lambda-(pG1*lambda/(2.*PI))*Integral; // 1.0 here comes from assumption that d_12 = d_1S !
...
double exactDiffusionIntegrand_conservationLaw(double x, void* params)
{
	...
	if ((pG1*eta) >= 0.00000001)
	{
		if ((pG1*eta) >= 2.5)
		{
			KernelSource = sqrt(PI/(2.*pG1*lambda*eta))*2.*exp( -pG1*lambda*( (yObs-y)+ eta ))*ratioDc;
		}
		else 
		{
			KernelSource = 2.*exp(-(pG1*lambda*(yObs-y)))*gsl_sf_bessel_K0(pG1*lambda*eta)*ratioDc;
		}
		Kernel = KernelSource;
	}
...
double exactDiffusionIntegrand_l1l2(double y, void* params)
{
	...
	locEq = -(kappaLoc/lambda)*sigmaLoc;// multiplied by ratioDc when returned value defined
	...
	if ((pG1*eta) >= 0.00000001)
	{
		if ((pG1*eta) >= 4.)
		{
			...
			Kernel1sidedPart = locEq*sqrt(PI/(2.*pG1*lambda*eta))*exp( -pG1*lambda*( (yObs-y)+eta ) )*( ((-1.*(xObs-x)+dxdy*(yObs-y))/eta)*(1.+(3./8.)*(1./(pG1*lambda*eta))) - dxdy*1. );
		...
		else
		{
			...
			Kernel1sidedPart = locEq*exp(-(pG1*lambda*(yObs-y)))*( ( (-1.*(xObs-x)+dxdy*(yObs-y) )/eta)*gsl_sf_bessel_K1(pG1*lambda*eta) - dxdy*gsl_sf_bessel_K0(pG1*lambda*eta) );
		...
		{
			KernelSource = dxdy*sqrt(PI/(2.*pG1*lambda*eta))*2.*exp( -pG1*lambda*( (yObs-y)+ eta )); 		}
		else 
		{
			KernelSource = dxdy*2.*exp(-(pG1*lambda*(yObs-y)))*gsl_sf_bessel_K0(pG1*lambda*eta);
		}
		//Kernel = KernelSource;
...
double exactDiffusionIntegrand_locEq(double x, void* params)
{
	...
	locEq = 1.*(deltaG1-kappaLoc*sigmaLoc/lambda);
	...
	if (((pG1*eta) >= 0.00000001) && (fabs(x)>0.000001))
	{
		if ((pG1*eta) >= 4.)
		{
			...
			Kernel1sidedPart = locEq*sqrt(PI/(2.*pG1*lambda*eta))*exp( -pG1*lambda*( (yObs-y)+eta ) )*( ((-dydx*(xObs-x)+(yObs-y))/eta)*(1.+(3./8.)*(1./(pG1*lambda*eta))) - 1. );


		}
		else
		{
			Kernel1sidedPart = locEq*exp(-(pG1*lambda*(yObs-y)))*( ((-dydx*(xObs-x)+(yObs-y))/eta)*gsl_sf_bessel_K1(pG1*lambda*eta) - gsl_sf_bessel_K0(pG1*lambda*eta) );
...
////////////////////////////// for right branch x>0 //////////////////////
	...
	for (int sndGridIndex = 0; sndGridIndex < addGridPoints; ++sndGridIndex)
	{
		...
		prolonguedXGridG12[sndGridIndex+numberGridPoints_y+addGridPoints] = 1.;//gsl_vector_get(x,numberGridPoints);	
	...
	prolonguedXGridG12[addGridPoints+numberGridPoints_y-1] = 1.;//gsl_vector_get(x,numberGridPoints); 
